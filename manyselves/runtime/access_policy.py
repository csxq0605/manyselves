"""Hard file-access boundaries shared by Agent-facing tools."""
...
def reject_forbidden_agent_document(
    ...
    """Prevent non-distiller Agents from opening the isolated source or snapshot."""

    if is_isolated_distillation_document(path_or_ref) and not allow_template_distiller:
        raise PermissionError(
            f"{FORBIDDEN_AGENT_ACCESS_ERROR}: this document is outside every Agent's "
            "read, search, inspection, and analysis scope except the isolated "
