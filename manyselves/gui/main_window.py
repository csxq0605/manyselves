...
class MainWindow(QMainWindow):
    ...
    def _apply_window_shadow(self) -> None:
        ...
        if sys.platform == "win32":
            # Windows keeps the native DWM shadow until custom chrome is added.
            pass
    ...
    def _setup_menu_bar(self, menubar: QMenuBar) -> None:
        ...
        file_menu.addSeparator()
        quit_act = QAction(f"退出 {PRODUCT_NAME}", self)
    ...
    def _on_open_folder(self) -> None:
        ...
        # Validate it looks like a Manyselves project; offer to scaffold if not.
        if not is_valid_project(folder):
            reply = question_box(
                self,
                "不是有效的项目",
                f"'{folder.name}' 不是 {PRODUCT_NAME} 项目。是否在该目录创建项目结构并打开？",
        ...
        QProcess.startDetached(
            sys.executable,
            ["-m", "manyselves", "--project", str(folder)],
        )
