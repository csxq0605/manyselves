...
from manyselves.core.file_search import FileSearchManager
from manyselves.core.input_history import InputHistoryStore
from manyselves.gui.scale import scaled
...
from manyselves.gui.widgets.file_search_popup import FileSearchPopup
...
from manyselves.gui.widgets.ui_utils import (
    IconActionButton,
    NoWheelComboBox,
    install_compact_tooltip,
    render_svg_icon,
)
from manyselves.interfaces.types import ApiDebugMessage
from manyselves.utils.agent_labels import get_agent_badge, get_agent_icon, get_agent_title
from manyselves.utils.logging_config import ui_logger
...
class AgentPanel(QWidget):
    ...
    def __init__(self, panel_id: str, title: str, workspace: Path | None = None):
        ...
        self._input_history_store = InputHistoryStore(self._workspace)
        self._drafts_by_agent: dict[str, str] = {}
    ...
    SLASH_COMMAND_SPECS = {
        ...
        "/init": "初始化项目报告资源",
        "/history": "打开会话历史",
        "/status": "查看当前 Agent 与队列状态",
        "/retry": "重新发送上一条普通消息",
        "/stop": "中断当前任务",
    }
    SLASH_COMMANDS = list(SLASH_COMMAND_SPECS)
    ...
    def _on_command_palette_requested(self, query: str, position: QPoint) -> None:
        ...
        matches = [
            cmd
            for cmd, description in self.SLASH_COMMAND_SPECS.items()
            if q in cmd.lower() or q in description.lower()
        ]

        self._cmd_popup.clear()
        for cmd in matches:
            item = QListWidgetItem(f"  {cmd}    {self.SLASH_COMMAND_SPECS[cmd]}")
    ...
    def _execute_slash_command(self, cmd: str, original_text: str) -> None:
        ...
        if cmd == "/help":
            help_text = "可用命令：\n" + "\n".join(
                f"  {name} — {description}"
                for name, description in self.SLASH_COMMAND_SPECS.items()
            )
            help_text += "\n\n快捷键：↑/↓ 输入历史；Tab 选择补全；Shift+Enter 换行。"
            ...
            queue = len(self._queued_messages)
            self.add_message(
                "agent",
                f"当前 Agent：{get_agent_title(self._agent_type)}\n"
                f"状态：{status}\n等待队列：{queue} 条",
            )
        elif cmd == "/retry":
            previous = self._last_sent_content or next(
                (
                    item
                    for item in reversed(self._input_field.input_history())
                    if not item.startswith("/")
                ),
                None,
            )
            if previous:
                self._last_sent_content = previous
                self._send_content(previous)
            else:
                self.add_message("agent", "没有可重试的上一条普通消息。")
        elif cmd == "/stop":
            if self._is_working:
                self.interrupt_requested.emit()
            else:
                self.add_message("agent", "当前没有正在运行的任务。")
        else:
            self.add_message("agent", f"未知命令：{cmd}。输入 /help 查看可用命令。")
    ...
    def set_agent_type(self, agent_type: str) -> None:
        if hasattr(self, "_input_field"):
            self._drafts_by_agent[self._agent_type] = self._input_field.get_plain_text()
        self._agent_type = agent_type
        if hasattr(self, "_input_field"):
            self._reload_input_history()
            self._input_field.set_text(self._drafts_by_agent.get(agent_type, ""))
        # Update icon
    ...
    def _on_send(self) -> None:
        ...
        self._record_input_history(content)
    ...
    def _reload_input_history(self) -> None:
        if not hasattr(self, "_input_field"):
            return
        self._input_field.set_input_history(
            self._input_history_store.entries(self._agent_type)
        )

    def _record_input_history(self, content: str) -> None:
        entries = self._input_history_store.append(self._agent_type, content)
        self._input_field.set_input_history(entries)
