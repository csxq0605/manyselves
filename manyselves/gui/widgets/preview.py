...
from PyQt6.QtCore import QEvent, QFileSystemWatcher, QSignalBlocker, Qt, QTimer, QUrl, pyqtSignal
from PyQt6.QtGui import QColor, QDesktopServices, QImage, QPixmap, QTextDocument
...
_WORD_SUFFIXES = frozenset({".docx", ".doc"})
...
def _create_scintilla(path: Path, lexer_name: str) -> tuple:
    ...
    try:
        ...
        if lexer_name == "QsciLexerTeX":
            # TeX: unified post-styling — applies \command, % comment, and
            # $math$ colouring in a single pass so they don't overwrite
            # each other (each call to SCI_COLOURISE resets prior styling).
            from ..scintilla_utils import attach_tex_post_styling
...
def _docx_paragraph_html(paragraph) -> str:
    """Render one python-docx paragraph into conservative Qt rich text."""
    text = html.escape(paragraph.text or "").replace("\n", "<br>")
    ...
    if normalized_style.startswith("heading"):
        try:
            level = max(2, min(5, int(style_name.rsplit(" ", 1)[-1]) + 1))
        except ValueError:
            level = 3
...
def _docx_table_html(table) -> str:
    ...
    for row_index, row in enumerate(table.rows):
        ...
        cells = "".join(
            f"<{tag}>{html.escape(cell.text or '').replace(chr(10), '<br>')}</{tag}>"
...
def _docx_image_resources(paragraph, start_index: int) -> tuple[list[tuple[QUrl, QImage]], str]:
    """Extract inline DOCX images as scaled QTextDocument resources."""
    from docx.oxml.ns import qn

    resources: list[tuple[QUrl, QImage]] = []
    tags: list[str] = []
    for blip in paragraph._p.xpath(".//a:blip"):  # noqa: SLF001
        ...
        resource_url = QUrl(f"docx-image:///{start_index + len(resources) + 1}")
        resources.append((resource_url, image))
        tags.append(
            f"<p class='image'><img src='{resource_url.toString()}' "
            f"width='{image.width()}' height='{image.height()}'></p>"
        )
    return resources, "".join(tags)


def _create_docx_viewer(path: Path) -> tuple:
    """Create a readable in-app DOCX preview with paragraphs, tables, and images."""
    ...
    browser.setObjectName("fileDocxViewer")
    browser.setOpenExternalLinks(False)
    browser.setStyleSheet(f"""
        QTextBrowser#fileDocxViewer {{
            background-color: {c["editor_bg"]};
            color: {c["fg"]};
            border: none;
            padding: 18px 28px;
    ...
    try:
        ...
        fragments = [
            ...
            "h1 { text-align: center; margin: 16px 0 20px 0; }"
            "h2, h3, h4, h5 { margin: 16px 0 8px 0; }"
            "p.list { margin-left: 18px; }"
            "p.image { text-align: center; margin: 12px 0; }"
        ...
        for child in document.element.body.iterchildren():
            if isinstance(child, CT_P):
                ...
                fragments.append(_docx_paragraph_html(paragraph))
                images, image_html = _docx_image_resources(paragraph, len(resources))
                resources.extend(images)
                fragments.append(image_html)
            elif isinstance(child, CT_Tbl):
                fragments.append(_docx_table_html(Table(child, document)))
        rich_document = browser.document()
        for url, image in resources:
            rich_document.addResource(QTextDocument.ResourceType.ImageResource, url, image)
        rich_document.setHtml("".join(fragments))
    except Exception as exc:
        logger.exception("Failed to preview DOCX: {}", path)
        browser.setPlainText(f"Word 文档预览失败：{path.name}\n\n{exc}")
    return browser, "docx"


def _create_legacy_word_viewer(path: Path) -> tuple:
    label = QLabel(f"旧版 Word 文档无法在应用内解析：{path.name}\n请点击右上角按钮用 Word 打开。")
    label.setObjectName("editorPlaceholder")
    label.setAlignment(Qt.AlignmentFlag.AlignCenter)
    return label, "word"


def _open_external_file(path: Path) -> bool:
    """Open a local file with the OS-associated application."""
...
def _create_viewer(path: Path) -> tuple:
    ...
    if suffix == ".docx":
        return _create_docx_viewer(path)
    if suffix == ".doc":
        return _create_legacy_word_viewer(path)
...
class PreviewWidget(QWidget):
    ...
    def __init__(self, workspace: Path):
        ...
        self._tab_state_path = self.workspace / ".manyselves" / "open_tabs.json"
    ...
    def _update_file_actions(self) -> None:
        ...
        if suffix == ".py":
            ...
            self._active_action_kind = "word"
            install_compact_tooltip(self._preview_button, "用 Word 或系统应用打开")
    ...
    def _on_preview_clicked(self) -> None:
        if not self._current_file:
            return
        if self._active_action_kind == "word" and not _open_external_file(self._current_file):
            warning_box(self, "打开失败", f"无法使用系统应用打开：\n{self._current_file}")
