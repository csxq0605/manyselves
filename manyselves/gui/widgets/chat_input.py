...
class ChatInput(QPlainTextEdit):
    ...
    def _sync_after_text_change(self) -> None:
        if self._history_index is not None and not self._setting_history_text:
    ...
    def keyPressEvent(self, event: QKeyEvent) -> None:
        ...
        if (
            ...
            and self._should_navigate_history()
        ):
            direction = "up" if key == Qt.Key.Key_Up else "down"
            if self.navigate_history(direction):
                return
    ...
    def _should_navigate_history(self) -> bool:
        """Keep normal caret movement for multiline or selected text."""
        ...
        return self._content_pixel_height() <= line_height * 1.5

    def set_input_history(self, entries: list[str]) -> None:
        """Replace available history entries, ordered oldest to newest."""
    ...
    def navigate_history(self, direction: str) -> bool:
        """Move through history and restore the unsent draft at the newest edge."""

        if direction not in {"up", "down"} or not self._input_history:
            return False
