...
_NON_DRAGGABLE_DIRS: set[str] = set()
...
class FileTreeWidget(QWidget):
    ...
    def __init__(self, workspace: Path):
        ...
        self._tree_state_path = self.workspace / ".manyselves" / "file_tree_state.json"
    ...
    def _init_directories(self) -> None:
        for dir_name in FIXED_DIRECTORIES:
            ...
            nested_names = {
                "Work": ("runs",),
                "Outputs": ("Modules", "Reviews", "Reports"),
            }.get(dir_name, ())
            for child_name in nested_names:
                ...
                child_item.setData(0, Qt.ItemDataRole.UserRole, f"{dir_name}/{child_name}")
                self._mark_fixed_directory_item(child_item)
                self._show_directory_indicator(child_item)
    ...
    def _get_selected_dir(self) -> str:
        """Get the selected directory path, falling back to ``Inputs``."""
        item = self.tree.currentItem()
        if not item:
    ...
    def _start_inline_create(self, dir_name: str, is_folder: bool, style: QStyle) -> None:
        ...
        # Find the target directory item (for example Outputs/Reports).
        target_item = None
