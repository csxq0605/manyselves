...
<submission_contract>
只有会实质改变项目事实、风险判断或行动建议的问题才能标记为 blocking；文字风格、细节增强和已透明保留的不确定性应标记为 warning。每个 blocking ReviewIssue 必须填写合法 submodule_id、受影响 Claim ID、当前证据定位、阻塞原因、可验证的关闭条件和责任 Agent。只有责任人提供关闭说明及验证证据后才能标记 resolved。返工范围只能覆盖这些被点名的子模块，不得用整章返工替代局部问题定位。
</submission_contract>
<deliverables>
