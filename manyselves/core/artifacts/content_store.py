...
class ContentAddressedStore:
    ...
    def promote_existing_to_view(
        ...
        *,
        ...
        """Atomically replace a verified materialized view with a CAS symlink.

        Promotion is deliberately stricter than :meth:`link_view`: it must
        publish a symbolic link (and therefore a real storage saving), or
        leave the existing target untouched.  The existing bytes are checked
        against the receipt digest before any replacement is attempted.
        """

        blob_path = self.resolve_blob(
            blob_ref,
            expected_sha256=expected_sha256,
        )
        target = self._require_mutable_view_path(target)
        if not target.is_file():
            raise FileNotFoundError(f"content view target is missing: {target}")
        if target.is_symlink():
            if target.resolve() != blob_path:
                raise ValueError(
                    "existing content view does not reference the expected blob"
                )
            return ContentView(path=target, mode="symlink")
        if target.resolve() == blob_path:
            raise ValueError("canonical content blob cannot be promoted to a view")
        actual_sha256, actual_size = self._digest(target)
        if actual_sha256 != expected_sha256:
            raise ValueError(
                "content view target does not match the expected SHA-256"
            )

        blob = self._blob(expected_sha256, actual_size, blob_path)
        staged = target.with_name(
            f".{target.name}.{uuid.uuid4().hex}.cas-view"
        )
        try:
            view = self.link_view(blob, staged, final_path=target)
            if view.mode != "symlink" or not staged.is_symlink():
                raise OSError("CAS view promotion requires symbolic-link support")
            os.replace(staged, target)
            self._fsync_directory(target.parent)
        finally:
            staged.unlink(missing_ok=True)
        if not target.is_symlink() or target.resolve() != blob_path:
            raise OSError("CAS view promotion did not publish the expected symlink")
        return ContentView(path=target, mode="symlink")
    ...
    def materialize_view(
        ...
        *,
        ...
        """Atomically restore a verified CAS view as an ordinary file."""
        ...
        if target.is_symlink():
            ...
            actual_sha256, _size = self._digest(target)
            if actual_sha256 != expected_sha256:
                raise ValueError(
                    "materialized target does not match the expected SHA-256"
                )
            return target
        ...
        actual_sha256, _size = self._digest(target)
        if target.is_symlink() or actual_sha256 != expected_sha256:
            raise OSError("materialized content failed post-write validation")
        return target
