"""Canonical Manyselves project directory layout and migration helpers."""
...
def is_project_workspace(path: Path) -> bool:
    ...
    return any(
        ...
        for marker in TOP_LEVEL_PROJECT_DIRECTORIES
    )
...
def _rename_directory_case(source: Path, target: Path) -> Path:
    ...
    tmp = source.parent / f".manyselves-case-{os.getpid()}-{target.name}"
    while tmp.exists():
