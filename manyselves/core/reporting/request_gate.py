...
class ReportingBlockedError(RuntimeError):
    def __init__(self, missing_evidence: list[str], affected_modules: list[str] | None = None):
    ...
    def evaluate(request: ReportRequest, coverage: CoverageMatrix) -> GateDecision:
        ...
        return GateDecision(
            ...
            affected_modules=[
                ...
                for module_id in request.target_modules
                if any(
                    submodule.status is not CoverageStatus.READY
                    for submodule in coverage.entries[module_id].submodules.values()
                )
            ],
        )
