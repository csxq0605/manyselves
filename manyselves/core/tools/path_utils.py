...
_CANONICAL_TOP_DIRS = ("Inputs", "Knowledge", "Templates", "Work", "Outputs")
_INTERNAL_METADATA_DIRS = (".manyselves", ".checkpoints")
...
def is_internal_metadata_path(file_path: Path, workspace: Path) -> bool:
    """Check if path is in internal metadata directories (.manyselves, .checkpoints).
    ...
    Returns:
        True if path is within .manyselves or .checkpoints, False otherwise.
    """
    try:
...
def is_internal_metadata_rel(rel_posix: str) -> bool:
    ...
    Args:
        rel_posix: Relative path as POSIX string (e.g., ".manyselves/file" or "Data/test.csv").

    Returns:
        True if path is within .manyselves or .checkpoints, False otherwise.
    """
    return (rel_posix in _INTERNAL_METADATA_DIRS or
            rel_posix.startswith(".manyselves/") or
