    ...
    def _check_write_permission(self, file_path: Path, action: str = "Write") -> None:
        if is_internal_metadata_path(file_path, self.workspace):
            raise PermissionError(
                f"{action} not allowed in internal metadata directories (.manyselves, .checkpoints). Attempted: {file_path}"
            )
...
class ReadTool(Tool):
    ...
    async def __call__(
        ...
        if is_internal_metadata_path(file_path, self.workspace):
            raise PermissionError("Access to internal metadata under .manyselves is not allowed.")
