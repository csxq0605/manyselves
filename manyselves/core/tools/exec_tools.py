...
class ExecTool(Tool):
    ...
    def _check_blocked_paths_in_command(self, command: str, tokens: list[str]) -> None:
        ...
        for token in tokens:
            ...
            if is_internal_metadata_rel(normalized):
                raise ValueError(
                    "Access to internal metadata directories (.manyselves, .checkpoints) is not allowed."
                )

        # Also check the raw command string for patterns
        for prefix in (".manyselves", ".checkpoints"):
            patterns = [
    ...
    def _filter_ls_output(self, output: str) -> str:
        """Filter internal metadata directories (``.manyselves``/``.checkpoints``) from ``ls`` output.
    ...
    def _filter_find_output(self, output: str) -> str:
        ...
        ``find`` prints one relative path per line (e.g. ``./.manyselves/file``).
