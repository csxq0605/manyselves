...
@pytest.mark.asyncio
async def test_real_three_workbooks_generate_complete_five_module_report(tmp_path: Path) -> None:
    inputs = tmp_path / "Inputs"
    inputs.mkdir()
    for filename in CORE_FILES:
        shutil.copy2(REAL_INPUT_DIR / filename, inputs / filename)
    ...
    result = await service.run(
        ReportRequest(
            instruction="根据三张核心工作表生成完整配电安全专家咨询报告",
    ...
    assert {row["module_id"] for row in evidence_rows if row["module_id"]} == {
        "2.1",
        "2.2",
        "2.3",
        "2.4",
    ...
    state = json.loads((tmp_path / "Work" / "report-state.json").read_text(encoding="utf-8"))
    assert [draft["module_id"] for draft in state["module_drafts"]] == [
        "2.1",
        "2.2",
        "2.3",
        "2.4",
    ...
    assert all(draft["approved"] for draft in state["module_drafts"])
    assert state["editorial"]["risk_summary"]
    assert state["editorial"]["actions"]
    assert state["skill_versions"]
    assert len(state["module_executions"]) == 5
    assert not any(issue["severity"] == "blocking" for issue in state["review_issues"])
    report_path = tmp_path / "Outputs" / "Reports" / "配电安全专家咨询报告.docx"
    assert report_path.stat().st_size > 1_000_000
