...
| 09 / Task 1 | Global knowledge file service and API | Implemented; awaiting commit | Fixed hidden data-root storage, logical safe-file API, session authentication and control-lease mutations are implemented. Gates: service/API/OpenAPI 21 passed, existing project-file regression 25 passed, targeted Ruff passed |
| 09 / Tasks 2-4 | Composite retrieval, run provenance, React page | Pending | Do not start before Task 1 passes its focused service/API/OpenAPI gates and is committed |
