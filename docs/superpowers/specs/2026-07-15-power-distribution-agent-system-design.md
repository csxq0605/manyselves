...
`/Users/zzymima0000/Documents/Codex/work/配电安全报告工具V2-交接/知识库源目录/01_页面导入知识库`

`01` 是 Agent 可主动搜索的外部参考文档库，定位类似联网搜索，而不是身份、system prompt、隐藏规则包或每轮固定注入的模块上下文。

Agent 可以根据当前问题搜索 `01`，用它：
...
Agent 必须判断检索结果是否适用于当前项目，不得直接复制知识库中的项目结论或固定句式。`01` 来源进入 `SourceLedger`，使用 `R-*` 标识，不进入 `EvidenceItem`。

### 3.3 `02_本地skill提示词资料_禁止导入`

目录：

`/Users/zzymima0000/Documents/Codex/work/配电安全报告工具V2-交接/知识库源目录/02_本地skill提示词资料_禁止导入`
...
- 不注入 Agent 上下文；
- 不让报告 Agent 在运行时读取；
- 不复制其中的客户名、位置、柜号、数值、图片或原始示例。

它只在开发阶段用于理解旧系统希望解决的质量问题，并离线抽象为身份能力、匿名正反例、断言和评测样本。
...
是否搜索 `01`、是否联网、是否向其他 Agent 提问、是否直接开始写作以及何时结束，由 Agent 在自己的 Loop 内决定。外层程序不得把这些选择实现成固定研究状态机。
...
`search_reference_library` 只搜索 `01`。`web_search` 和 `open_web_source` 必须保存可引用元数据。研究工具是否调用由 Agent 决定。
