...
| `full_report` | 从 `Inputs/` 重新生成完整报告 | 资料准备 → 三波协作与五模块并行写作 → 2.1–2.5 独立审查顺序闭环 → 跨模块审查 → 总编 → 成稿审计 → 渲染 |
...
完整报告目前先让五个模块并行完成接口发现，经两道确定性屏障形成协作包，再并行完成专业写作。之后 2.1、2.2、2.3、2.4、2.5 的独立责任审计、定向修改和原审查者复核仍按固定模块顺序执行；五个模块全部闭环后，才开始跨模块审查和总编。
