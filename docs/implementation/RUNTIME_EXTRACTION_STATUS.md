...
- Current slice: `taxonomy 与 Reporting 基础模型/ReportRequest 已迁入 Capability；下一步整文件迁移 agentic schema registry，再继续 submission→input`
...
| Reporting Python 归 Capability 所有 | Module Lane/Cohort、Cross Owner、Chief/Final Chapter、Final Review、taxonomy 以及 Reporting 基础模型/ReportRequest 已迁入 Capability；其中包装模型仍引用待迁移的 Core agentic/input 类型，Agent runner、review/render/delivery 也仍在 `manyselves/core/reporting` | 基础领域模型已真实归位，但 agentic schema registry 与完整领域实现尚未归位 | FA-03/FA-05 接下来迁移 agentic→submission→input 领域 DAG 与执行实现，而非继续停留在外层 wrapper |
