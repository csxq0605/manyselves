#!/usr/bin/env python3
"""Calculate model-aware MiMo-V2.5 Token Plan/API cost for a report run."""
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="按 .env/参数中的 MiMo-V2.5 模型换算四档 Token Plan 和国内 API 成本。"
    )
    ...
    parser.add_argument(
        ...
        help="覆盖 .env 的 MANYSELVES_BOOTSTRAP_MODEL/MIMO_MODEL",
    )
    parser.add_argument(
        "--off-peak",
        action="store_true",
...
def main() -> int:
    ...
    if args.run_id:
        report = calculate_mimo_run_cost(
            args.workspace,
            args.run_id,
            default_model=args.model or configured_mimo_model(),
        )
    else:
        timestamp = (
        ...
        report = calculate_mimo_cost(
            [{
                "timestamp": timestamp,
                "model": args.model or configured_mimo_model() or "mimo-v2.5-pro",
    ...
    if report is None:
        raise SystemExit("没有找到可计价的 mimo-v2.5/mimo-v2.5-pro usage 记录。")
    if args.json:
        print(json.dumps(report, ensure_ascii=False, indent=2))
    else:
        print(format_mimo_cost(report))
    return 0
